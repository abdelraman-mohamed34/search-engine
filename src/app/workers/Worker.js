...
self.onmessage = async (e) => {
    ...
    getRequest.onsuccess = async () => {
        ...
        if (finalData.length === 0) {
            ...
            finalData.forEach(item => saveStore.put(item));
        } else {
        ...
        for (let i = 0; i < finalData.length; i += chunkSize) {
            ...
            const avg = chunkProducts.reduce((acc, p) => acc + Number(p.price), 0) / chunkSize;
            totalAvg.push(avg.toFixed(2));
        }
